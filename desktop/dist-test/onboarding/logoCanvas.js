...
exports.THERMAL_STYLES = exports.RECEIPT_LOGO_WIDTHS = exports.RECEIPT_LOGO_WIDTH = exports.MAX_IMAGE_BYTES = exports.MONOGRAM_STYLES = exports.LOGO_SIZE = void 0;
exports.shade = shade;
exports.readableTextOn = readableTextOn;
exports.flipImage = flipImage;
exports.cropImage = cropImage;
exports.circleMaskGeometry = circleMaskGeometry;
exports.applyShapeMask = applyShapeMask;
exports.removeWhiteBackground = removeWhiteBackground;
exports.contentBounds = contentBounds;
exports.autoTrimImage = autoTrimImage;
exports.tightenImage = tightenImage;
exports.imageFileProblem = imageFileProblem;
exports.shrinkOversizedImage = shrinkOversizedImage;
exports.renderMonogram = renderMonogram;
exports.buildReceiptLogoVariants = buildReceiptLogoVariants;
exports.thermalStyleById = thermalStyleById;
exports.renderThermalPreview = renderThermalPreview;
exports.buildLogoVariants = buildLogoVariants;
exports.initialsFrom = initialsFrom;
/** Сторона квадратного логотипа. 512 хватает и для чека (384 точки), и для UI. */
exports.LOGO_SIZE = 512;
exports.MONOGRAM_STYLES = [
...
function createCanvas(width, height) {
    ...
    if (!context)
        throw new Error('Не удалось подготовить холст для логотипа.');
...
function roundedPath(context, size, radius) {
    context.beginPath();
 ...
 * видел на экране.
 */
async function flipImage(imageUrl, axis) {
...
async function cropImage(imageUrl, area, rotationDegrees = 0) {
    const image = await loadImage(imageUrl);
    ...
    const [canvas, context] = createCanvas(exports.LOGO_SIZE, exports.LOGO_SIZE);
    ...
    */
    const scale = Math.min(exports.LOGO_SIZE / cropWidth, exports.LOGO_SIZE / cropHeight);
    const drawWidth = cropWidth * scale;
    const drawHeight = cropHeight * scale;
    context.drawImage(source, cropX, cropY, cropWidth, cropHeight, (exports.LOGO_SIZE - drawWidth) / 2, (exports.LOGO_SIZE - drawHeight) / 2, drawWidth, drawHeight);
    return canvas.toDataURL('image/png');
}
/* -------------------------------------------------------------------------- */
 ...
 */
function circleMaskGeometry(width, height) {
    const side = Math.min(width, height) || exports.LOGO_SIZE;
    return {
        side,
 ...
 */
async function applyShapeMask(source, shape) {
 ...
 */
async function removeWhiteBackground(source, threshold = 240) {
    if (!source)
        return source;
    const image = await loadImage(source);
 ...
 */
function contentBounds(data, width, height, tolerance = TRIM_TOLERANCE) {
    if (width <= 0 || height <= 0)
        return null;
    const at = (x, y) => {
        const p = (y * width + x) * 4;
    ...
    const isBackground = (x, y) => {
        ...
        if (pixel.a < 24)
            return true;
        return (Math.abs(pixel.r - background.r) <= tolerance &&
 ...
 * исходник.
 */
async function autoTrimImage(source, padRatio = 0.02) {
    if (!source)
        return '';
    const image = await loadImage(source);
    const width = image.naturalWidth;
 ...
 * вызова.
 */
async function tightenImage(image) {
    if (!image)
        return image;
 ...
 */
exports.MAX_IMAGE_BYTES = 32 * 1024 * 1024;
/**
...
function imageFileProblem(file) {
    ...
    if (file.size > exports.MAX_IMAGE_BYTES) {
        return `Файл больше ${Math.round(exports.MAX_IMAGE_BYTES / 1024 / 1024)} МБ — это не картинка логотипа.`;
    }
    return '';
}
 ...
 */
async function shrinkOversizedImage(file, maxSide = 2048) {
    ...
    try {
        ...
        const [canvas, context] = createCanvas(Math.round(image.naturalWidth * scale), Math.round(image.naturalHeight * scale));
        context.imageSmoothingQuality = 'high';
    ...
    try {
        ...
        const scale = maxSide / longest;
        const [canvas, context] = createCanvas(Math.round(bitmap.width * scale), Math.round(bitmap.height * scale));
        context.imageSmoothingQuality = 'high';
...
function renderMonogram(letters, color, style) {
    const [canvas, context] = createCanvas(exports.LOGO_SIZE, exports.LOGO_SIZE);
    const text = (letters || 'K').slice(0, 2).toUpperCase();
    context.fillStyle = '#ffffff';
    context.fillRect(0, 0, exports.LOGO_SIZE, exports.LOGO_SIZE);
    if (style === 'circle') {
        context.beginPath();
        context.arc(exports.LOGO_SIZE / 2, exports.LOGO_SIZE / 2, exports.LOGO_SIZE / 2 - 4, 0, Math.PI * 2);
        context.fillStyle = color;
        context.fill();
    }
    else if (style === 'outline') {
        roundedPath(context, exports.LOGO_SIZE, exports.LOGO_SIZE * 0.24);
        ...
        roundedPath(context, exports.LOGO_SIZE, exports.LOGO_SIZE * 0.24);
        if (style === 'gradient') {
            const gradient = context.createLinearGradient(0, 0, exports.LOGO_SIZE, exports.LOGO_SIZE);
    ...
    context.fillText(text, exports.LOGO_SIZE / 2, exports.LOGO_SIZE / 2 + exports.LOGO_SIZE * 0.03);
    return canvas.toDataURL('image/png');
}
/**
 ...
 */
exports.RECEIPT_LOGO_WIDTH = 384;
/**
 ...
 */
async function buildReceiptLogoVariants(mark, options = {}) {
    if (!mark)
        return { w384: '', w288: '' };
    const [w384, w288] = await Promise.all([
        renderThermalPreview(mark, { ...options, width: exports.RECEIPT_LOGO_WIDTHS['80'] }),
        renderThermalPreview(mark, { ...options, width: exports.RECEIPT_LOGO_WIDTHS['58'] }),
    ]);
    return { w384, w288 };
}
exports.THERMAL_STYLES = [
...
function thermalStyleById(id) {
    return exports.THERMAL_STYLES.find((item) => item.id === id) ?? exports.THERMAL_STYLES[0];
}
/**
 ...
 */
async function renderThermalOutline(logoDataUrl, options = {}) {
 ...
 */
async function renderThermalPreview(logoDataUrl, options = {}) {
    const style = options.style ?? 'standard';
    if (style === 'outline')
        return renderThermalOutline(logoDataUrl, options);
    const { threshold = thermalStyleById(style).threshold, width, 
    ...
    for (let y = 0; y < targetHeight; y += 1) {
        for (let x = 0; x < targetWidth; x += 1) {
            ...
            if (dither) {
                ...
                if (y + 1 < targetHeight) {
                    if (x > 0)
                        gray[index + targetWidth - 1] += (error * 3) / 16;
                    gray[index + targetWidth] += (error * 5) / 16;
                    if (x + 1 < targetWidth)
                        gray[index + targetWidth + 1] += error / 16;
                }
            }
            const p = index * 4;
            data[p] = next;
 ...
 */
async function resizeSquare(source, size) {
    const image = await loadImage(source);
 ...
 */
async function buildLogoVariants(mark) {
    if (!mark)
    ...
    const [s512, s128, s64, receipt] = await Promise.all([
        ...
        renderThermalPreview(mark, { width: exports.RECEIPT_LOGO_WIDTH }),
    ]);
    return { s512, s128, s64, receipt };
}
/** Инициалы для монограммы из названия магазина или компании. */
function initialsFrom(...candidates) {
