...
exports.ACCOUNT_STEP = exports.FIELDS = exports.SECTION_META = exports.FORM_STEPS = exports.LAST_STEP = exports.STEPS = void 0;
exports.normalizeCoordinate = normalizeCoordinate;
exports.sectionMeta = sectionMeta;
exports.fieldById = fieldById;
exports.fieldApplies = fieldApplies;
exports.readField = readField;
exports.writeField = writeField;
exports.validateField = validateField;
exports.fieldsForStep = fieldsForStep;
exports.sectionsForStep = sectionsForStep;
exports.groupBySection = groupBySection;
exports.problemsForStep = problemsForStep;
exports.optionalGapsForStep = optionalGapsForStep;
...
function coordinateError(value, limit, name) {
    if (!value.trim())
        return '';
    const parsed = Number(value);
    if (!Number.isFinite(parsed))
        return `${name}: нужно число, например 42.86622.`;
    if (Math.abs(parsed) > limit)
        return `${name} должна быть в пределах ±${limit}.`;
    return '';
}
function rateValidator(label) {
    return (value) => {
        if (!value.trim())
            return '';
        const parsed = Number(value);
        ...
        if (parsed < 0 || parsed > 100)
            return `${label} указывается в процентах, от 0 до 100.`;
        return '';
    };
}
...
exports.LAST_STEP = exports.STEPS.length - 1;
/** Сколько шагов считается «шагами» в счётчике: проверка в их число не входит. */
exports.FORM_STEPS = exports.LAST_STEP;
/**
...
exports.SECTION_META = {
    ...
    'Как считать аналитику': {
        caption: 'Какая цифра будет главной на дашборде. Учёт от этого не меняется — продажи и расходы пишутся всегда, и переключить можно в любой момент',
    },
    ...
    'Вход в систему': {
        caption: 'Email вместо логина и два разных пароля: один для входа в систему, второй — только для кабинета владельца с деньгами. Кассиров и их PIN заводят позже, в разделе «Сотрудники»',
    },
};
/** Разрешённая под режим подпись и признак «свёрнута по умолчанию». */
function sectionMeta(section, mode) {
    const meta = exports.SECTION_META[section] ?? {};
...
/* Реестр                                                                     */
/* -------------------------------------------------------------------------- */
const FISCAL_ONLY = ['fiscal'];
const SIMPLE_ONLY = ['simple'];
exports.FIELDS = [
    /* ── Шаг 1: режим работы ─────────────────────────────────────────────── */
    {
        id: 'fiscalMode',
    ...
    },
    {
        ...
        normalize: phone_1.phoneValue,
        validate: (value) => (0, phone_1.phoneProblem)(value),
    },
    {
        id: 'contacts.email',
        group: 'contacts',
    ...
    // самого контрола.
    {
        ...
        validate: (value, data) => {
            ...
            if (!lon.trim())
                return 'Долгота не определена — нажмите «Определить по адресу».';
    ...
    },
    {
        ...
        required: true,
        receiptLine: 'Валюта',
        options: dictionaries_1.CURRENCIES.map((item) => ({
            value: item.code,
    ...
    },
    {
        ...
        options: dictionaries_1.TIMEZONES.map((item) => ({ value: item.value, label: item.label })),
    },
    {
        id: 'business.country',
        group: 'business',
        ...
        section: 'Локализация',
        required: true,
        options: dictionaries_1.COUNTRIES.map((item) => ({ value: item, label: item })),
    },
    ...
    // подсказка. Ставки при этом остаются обычными полями и правятся руками.
    {
        ...
        required: true,
        receiptLine: 'СНО',
        options: types_1.TAX_REGIMES.map((item) => ({ value: item.id, label: item.label })),
        hintFor: (data) => `${(0, types_1.taxRegimeHint)(data.tax.regime)} Ставки подставлены автоматически — поправьте, если у вас иначе.`,
        afterWrite: (data) => (0, types_1.applyTaxRegime)(data, data.tax.regime),
    },
    {
    ...
    },
    {
        ...
        placeholder: '0',
        receiptLine: 'НДС %',
        hintFor: (data) => data.tax.regime === 'general'
    ...
    },
    {
        ...
        */
        validate: (value, data) => (0, applyTheme_1.accentProblem)(value, data.branding.theme),
    },
    {
...
/* -------------------------------------------------------------------------- */
const FIELD_BY_ID = new Map(exports.FIELDS.map((field) => [field.id, field]));
 ...
 */
function fieldApplies(field, data) {
    if (field.modes && !field.modes.includes(data.fiscalMode))
        return false;
...
function writeField(data, field, raw) {
    if (field.group === 'root') {
    ...
    if (typeof previous === 'boolean')
        next = raw === 'true';
    else if (field.kind === 'decimal' && typeof previous === 'number')
        next = toNumber(raw);
    const written = {
        ...data,
...
function fieldsForStep(step, data) {
    return exports.FIELDS.filter((field) => field.step === step && fieldApplies(field, data));
}
/** Секции шага в порядке объявления полей. */
function sectionsForStep(step, data) {
...
function problemsForStep(step, data) {
    const problems = [];
...
function optionalGapsForStep(step, data) {
    return fieldsForStep(step, data).filter((field) => !field.required &&
        !field.virtual &&
 ...
 */
exports.ACCOUNT_STEP = exports.FIELDS.find((field) => field.id === 'owner.password').step;
