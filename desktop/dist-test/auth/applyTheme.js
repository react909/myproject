...
exports.MIN_TEXT_CONTRAST = exports.DEFAULT_PRIMARY = void 0;
exports.isValidHex = isValidHex;
exports.normalizeHex = normalizeHex;
exports.sanitizeHexInput = sanitizeHexInput;
exports.darken = darken;
exports.lighten = lighten;
exports.contrastRatio = contrastRatio;
exports.readableTextOn = readableTextOn;
exports.accentTextOn = accentTextOn;
exports.accentProblem = accentProblem;
exports.accentOtherThemeWarning = accentOtherThemeWarning;
exports.applyTheme = applyTheme;
exports.readCachedTheme = readCachedTheme;
exports.cacheTheme = cacheTheme;
exports.applyAndCacheTheme = applyAndCacheTheme;
exports.applyStoredTheme = applyStoredTheme;
/** Стандартный акцент системы. Совпадает с дефолтом в types.ts и в базе. */
exports.DEFAULT_PRIMARY = '#00f5bc';
/**
...
function normalizeHex(value) {
    ...
    return isValidHex(trimmed) ? `#${trimmed.slice(1).toLowerCase()}` : exports.DEFAULT_PRIMARY;
}
 ...
 */
function accentTextOn(accent, surface, minContrast = exports.MIN_TEXT_CONTRAST) {
    const normalized = normalizeHex(accent);
    if (contrastRatio(normalized, surface) >= minContrast)
        return normalized;
 ...
 */
function accentProblem(accent, mode) {
    if (!isValidHex(accent))
        return 'Цвет задан не полностью — нужно шесть знаков после решётки.';
 ...
 */
function accentOtherThemeWarning(accent, mode) {
    const other = mode === 'dark' ? 'light' : 'dark';
    if (!accentProblem(accent, other))
        return '';
    return other === 'dark'
 ...
 */
function applyTheme({ mode, primary }) {
...
function readCachedTheme() {
    try {
        ...
        if (parsed?.mode !== 'light' && parsed?.mode !== 'dark')
            return null;
        if (typeof parsed.primary !== 'string' || !isValidHex(parsed.primary))
            return null;
 ...
 */
function applyAndCacheTheme(choice) {
    applyTheme(choice);
    cacheTheme(choice);
}
/**
