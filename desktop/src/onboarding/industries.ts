...
/** Тип поля атрибута — определяет, каким контролом он рисуется в карточке. */
export type AttributeKind = 'text' | 'number' | 'select' | 'date' | 'barcode'
...
export type ProductAttribute = {
  ...
  options?: string[]
  /** Показывать в списке товаров отдельной колонкой. */
  inList?: boolean
  required?: boolean
}

/** Шаблон карточки товара: чем отличается форма добавления. */
export type CardTemplate =
  /** Размерная сетка: одна модель — много строк размер/цвет. */
  | 'variants'
  /** Партия со сроком годности. */
  | 'batch'
  /** Весовой товар с печатью этикетки. */
  | 'weighted'
  /** Учёт по серийным номерам и гарантии. */
  | 'serial'
  /** Блюдо из состава. */
  | 'recipe'
  /** Услуга без остатков. */
  | 'service'
  /** Обычная штучная позиция. */
  | 'simple'

export type IndustryPreset = {
  id: IndustryId
  title: string
  /** Одна строка о том, что именно включится при выборе. */
  enables: string
  /** Единица по умолчанию — первая из `units`. */
  defaultUnit: string
  /**
   * Единицы, которые сфера разрешает. Не одна: в косметике рядом стоят
   * штучные флаконы и разливной парфюм, в продуктах — вес, штуки и литры.
   */
  units: string[]
  defaultKind: 'piece' | 'weight' | 'service'
  cardTemplate: CardTemplate
  attributes: ProductAttribute[]
  categories: string[]
  // Режима склада здесь нет: остатки ведутся всегда, независимо от сферы.
  // Услуга просто не имеет остатка — это свойство товара, а не настройка.
}
...
const BY_ID = new Map(INDUSTRIES.map((item) => [item.id, item]))

export function industryById(id: string): IndustryPreset {
  return BY_ID.get(id as IndustryId) ?? INDUSTRIES[INDUSTRIES.length - 1]
}

export const INDUSTRY_IDS = INDUSTRIES.map((item) => item.id)
